...
export function AuthProvider({ children }) {
  ...
  // Account creation runs server-side in a Cloud Function. Public sign-up is
  // disabled on the project, so the client SDK can't create users at all —
  // and shouldn't. The function verifies the caller is an admin against their
  // own users document before doing anything, which a client-side check can't
  // guarantee since anyone could call the endpoint directly.
  //
  // This also makes the auth user and the profile document a single unit: if
  // the profile write fails, the function deletes the login it just created
  // rather than leaving someone who can sign in but has no role.
